...
export default class extends Component {
  ...
  render() {
    ...
    const rowsCount = showBills ? store.getSize() : 0;
    ...
    return (
      <div styleName='product-items-wrapper bills-table'>
          ...
          <div styleName='products-line-1'>
            ...
            <ClickOutsideWrapper
              onClickOutside={(e) => {
                ...
                // }
                // store.hasActiveRow() &&
                this._setActiveRow(undefined, e);
              }}>
                    ...
                    cell={({rowIndex, ...props}) => {
                        return (
                          ...
                          <div>
                            <BillInfoCell
                            {...props}
                            onBill={this._onBill.bind(this, rowIndex)}
                            rowIndex={rowIndex}
                            styles={styles}
                            store={store}
                          />
  ...
  //   const { store, fields: { amountReceived, }, } = this.props;
...
class BillInfoCell extends Component {
  ...
  render() {
    ...
    const {bill} = store.getObjectAt(rowIndex);

    return (
      <Cell {...props}>
         <div>
          <span style={{marginLeft: 10, display: 'inline', overflow: 'hidden', textOverflow: 'ellipsis',}}
                className=''><a onClick={this.props.onBill}>{'Facture fournisseur'} {bill.refNo ? `nº${bill.refNo}` : ''}</a> ({moment(bill.date).format('ll')})</span>
        </div>
      </Cell>
    );
  }
    ...
    return (
      <Cell {...this.props}>
        <div>
          ...
          {getOriginalAmount(intl, item.bill)}
...
function getOpenBalance(intl, { bill: { total, balanceDue, }, amount, }){

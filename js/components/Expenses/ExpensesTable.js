...
class SortHeaderCell extends React.Component {
  ...
  render() {
    ...
    return (
      <Cell {...props} onClick={this._onSortChange} className={classnames(className, { [styles['has-key']]: sortKey === columnKey || getSortKey(columnKey) === sortKey || (columnKey === 'date' && !sortKey), })} columnKey={columnKey}>
        <a>
...
export default class ExpensesTable extends React.Component{
  ...
  static displayName = 'ExpensesTable';
  ...
  _onToggleAll = (e) => {
    ...
    sel.mode === SelectionModes.None || sel.mode === SelectionModes.Some
      ? toggleAll('expenses', page)
      : toggleNone('expenses', page);
  };

  _onToggle = (id, e) => {
    const { page, toggle, } = this.props;
    stopEvent(e);
    toggle('expenses', { key: id, page, }, this.state.store.getSize());
  };

...
class Expenses extends Component {
  ...
  render() {
    ...
    const data = loading || stale ? [] : loadData({ totalCount, offset, limit, sortKey, sortDir, company: { expenses: company.expenses, bills: company.bills, payments: company.payments, },});
      ...
      <div className=''>
        ...
        <div className='content'>
          ...
          <div styleName='page'>
            ...
            <div styleName='index' style={{/*height: 794 - 140Math.max(minHeight, 794, minHeight),*/ background: '#fff', position: 'absolute', top: 96, paddingBottom: 50, }}>
      ...
      viewer: () => Relay.QL`
        fragment on User {
          ...
          company(id: $companyId){
            ...
            expenses(first: 100000, offset: $offset, limit: $limit, type: $type, status: $status, sortKey: $sortKey, sortDir: $sortDir, from: $from, to: $to, payee: $payee, customer: $customer, _rev: $rev,){
              totalCount,
              sumOfTotals,
              edges{
                node {
            ...
            bills(first: 100000, offset: $offset, limit: $limit, type: $type, status: $status, sortKey: $sortKey, sortDir: $sortDir, from: $from, to: $to, payee: $payee, customer: $customer, _rev: $rev,){
              totalCount,
              sumOfTotals,
              sumOfBalances,
              edges{
            ...
            payments: paymentsOfBills(first: 100000, offset: $offset, limit: $limit, type: $type, status: $status, sortKey: $sortKey, sortDir: $sortDir, from: $from, to: $to, payee: $payee, customer: $customer, _rev: $rev,){
              totalCount,
              sumOfTotals,
              sumOfCredits,
              edges{
...
function loadData({ totalCount, offset, limit, sortKey, sortDir, company: { expenses, bills, payments, }, }){
...
function getTotalCount({ expenses, bills, payments, }){
  return expenses.totalCount + bills.totalCount + payments.totalCount;
}

function decorateExpense({ objectId, __dataID__, id, payee, paymentRef, creditToAccountCode, paymentMethod, mailingAddress, date, itemsConnection, memo, files, }) {
  return {
    __dataID__,
    id,
    date,
...
function decoratePayment({ objectId, __dataID__, id, mailingAddress, payee, paymentRef, amountReceived, creditToAccountCode, paymentMethod, date, itemsConnection, memo, files, }) {

...
class Item extends Component {
  ...
  _onPaymentVendorSelected = ({id}, onReadyStateChange) => {
    this.props.relay.setVariables({
      paymentVendorId: typeof id === 'undefined' ? this.props.vendor.objectId : id,
      rev: Revision.incrementAndGet(),
    }, onReadyStateChange);
  };
  ...
  render() {
    ...
    const totalCount = loading ? 0 : getTotalCount({ bills: payee.bills, expenses: payee.expenses, payments: payee.payments, });
    ...
    return (
      <div className={''} styleName={''}>
        ...
        <div className='content' styleName={'split-view flex-columns'} style={{ width: bodyWidth - 225, height: getBodyHeight() * 0.9, }}>
          ...
          <div styleName='page detail flex-flexible' style={drawerOpen ? { width: bodyWidth - 225 - DRAWER_WIDTH, } : {}}>
      ...
      viewer: () => Relay.QL`
          fragment on User {
            ...
            vendor: getVendor(companyId: $companyId, id: $id){
              ...
              ${ReceivePaymentOfBillsMutation.getFragment('_vendor')},
              ...
              payments: paymentsOfBills(first: 100000, offset: $offset, limit: $limit, type: $type, status: $status, sortKey: $sortKey, sortDir: $sortDir, from: $from, to: $to, _rev: $rev,){
                totalCount,
                sumOfTotals,
                sumOfCredits,
                edges{
...
function loadData({ totalCount, offset, limit, sortKey, sortDir, data: { bills, expenses, payments, }, }){
...
function decoratePayment({ objectId, __dataID__, id, payee, paymentRef, amountReceived, creditToAccountCode, paymentMethod, date, itemsConnection, memo, files, }) {

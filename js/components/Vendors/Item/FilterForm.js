...
export default class extends React.Component {
  ...
  render() {
    ...
    return (
      <Modal dialogClassName={`${styles['modal']} ${styles[`expenses-${typeValue}`]} ${styles['mini']} expenses-filter-form`}
                                    ...
                                    switch (value.id) {
                                      case 'recent':
                                        this.context.store.dispatch(
                                          batchActions([
                                            change('vendorExpensesFilterForm', formKey, 'type', value.id),
                                            change('vendorExpensesFilterForm', formKey, 'date', 12),
                                            change('vendorExpensesFilterForm', formKey, 'status', 'ALL'),
                                            change('vendorExpensesFilterForm', formKey, 'from', moment().startOf('day').subtract(30, 'days').format()),
                                      ...
                                      case 'open':
                                        this.context.store.dispatch(
                                          batchActions([
                                            change('vendorExpensesFilterForm', formKey, 'type', value.id),
                                            change('vendorExpensesFilterForm', formKey, 'status', 'open'),
                                          ])
                                        );
                                        break;
                                      case 'overdue':
                                        this.context.store.dispatch(
                                          batchActions([
                                            change('vendorExpensesFilterForm', formKey, 'type', value.id),
                                            change('vendorExpensesFilterForm', formKey, 'status', 'overdue'),
                                          ])
                                        );
                                        break;
                                      default:
                                        ...
                                        this.context.store.dispatch(
                                          batchActions([
                                            change('vendorExpensesFilterForm', formKey, 'type', value.id),
                                            change('vendorExpensesFilterForm', formKey, 'status', 'ALL'),
                                            change('vendorExpensesFilterForm', formKey, 'date', 13),
                            ...
                            <div className={''} id='date'>
                              ...
                              <Combobox
                                ...
                                onChange={item => {
                                  ...
                                  this.context.store.dispatch(
                                    batchActions([
                                      change('vendorExpensesFilterForm', formKey, 'date', item.id),

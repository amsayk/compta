...
export default class extends Component{
  ...
  _onDone = ({ type, status, date, from, to, customer, }) => {
    this.props.onFilter({
          ...
          <div styleName='breadcrumbs breadcrumb'>
            <span>
              ...
              {customer && customer !== 'ALL'
                ? function(){
                  hasFilter = true;
                  const index = topLoading ? -1 : find(self.props.company.customers.edges, ({node: {objectId}}) => objectId === customer);
                  return index === -1 ? null : renderLabel({
                      label: self.props.company.customers.edges[index].node.displayName,

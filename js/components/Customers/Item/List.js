...
export default class extends React.Component {
  ...
  constructor(props, context){
    ...
    this.state = {
      ...
      sortBy: localStorage.getItem('customer.list.sortBy') || 'displayName',
      modalOpen: false,
    };
  }
  _openModal = () => {
  ...
  _renderModal = () => {
    ...
    return (
      <CustomerForm
        ...
        onDone={({customer}) => {
          setTimeout(() => { this.context.router.push(`/apps/${this.props.company.id}/customer/${customer.id}`); }, 10);
        }}
      />
    );
  };
  _onSort = (key) => {
    if(this.state.sortBy !== key){
      this.setState({
        ...
        localStorage.setItem('customer.list.sortBy', key);
      });
    }
  };
  ...
  render(){
    ...
    const { company, loading, customer, styles, onToggle, } = this.props

    const customers = loading ? [] : loadData(company.customers.edges, { filterText, sortBy, })
            ...
            {open ? <div style={{ height: '100%', }}>
              ...
              <div styleName='list-stage'>
                ...
                <div styleName='top-stage'>
                  <a styleName='back-link inlineBlock' onClick={e => { stopEvent(e); setTimeout(() => { this.context.router.push(`/apps/${company.id}/customers`); }, 100); }}>
              ...
              <div styleName='directoryList list'>
                ...
                <div classname={'dgrid-list'} styleName='dgrid' role='grid' style={{ height: 52 * Math.min(13, customers.length), }}>
                  ...
                  <div styleName='dgrid-scroller' tabIndex='-1' style={{ WebkitUserSelect: 'none', marginTop: 0, marginBottom: 0, }}>
                    ...
                    <div styleName='dgrid-content'>
                      ...
                      {map(customers, ({ node: obj,}) => {
                        return (
                          <div className={classnames(styles['dgrid-row'], { [styles['dgrid-selected']]: obj.id === customer.id, })} tabIndex='0'>

                            <div className={styles['directory-list-item']}>

                              <a className={styles['link']} onClick={e => { stopEvent(e); setTimeout(() => { this.context.router.push(`/apps/${company.id}/customer/${obj.objectId}`); }, 100); }}>
                                <div className={styles['display-name']}>{obj.displayName}</div>
                                <div className={styles['balance']}>{intl.formatNumber(obj.salesStatus.open.amount, { format: 'MAD', })}</div>
                              </a>
...
function loadData(customers, { filterText, sortBy, }){
  function getSortKey() {
    switch(sortBy){
      case 'displayName': return (obj) => obj.node.displayName;
      case 'openBalance': return (obj) => obj.node.salesStatus.open.amount;
    }
  }

  if(typeof filterText !== 'undefined' && filterText !== null && filterText.length > 0){
    return orderBy(
      filter(customers, ({ node: obj}) => {
...
const messages = defineMessages({
  SortBy_displayName: {
    id: 'customers-List.sort-by-display-name',
    defaultMessage: 'Trier par nom',
  },

  SortBy_openBalance: {
    id: 'customers-List.sort-by-open-balance',

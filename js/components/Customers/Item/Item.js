...
class Item extends Component {
  ...
  render() {
    ...
    const totalCount = loading ? 0 : getTotalCount({ invoices: customer.invoices, sales: customer.sales, payments: customer.payments, });

    // const minHeight = getBodyHeight() - 236;

    const data = loading || stale ? [] : loadData({ totalCount, offset, limit, sortKey, sortDir, data : { invoices: customer.invoices, sales: customer.sales, payments: customer.payments, }, });
    ...
    return (
      <div className={''} styleName={''}>
        ...
        <div className='content' styleName={'split-view flex-columns'} style={{ width: bodyWidth - 225 + 2, height: getBodyHeight() * 0.9, }}>
          ...
          <div styleName='page detail flex-flexible' style={drawerOpen ? { width: bodyWidth - 225 - DRAWER_WIDTH, overflow: 'hidden', } : {}}>
      ...
      viewer: () => Relay.QL`
          fragment on User {
            ...
            customer: getCustomer(companyId: $companyId, id: $id){
              ...
              invoices(first: 100000, offset: $offset, limit: $limit, type: $type, status: $status, sortKey: $sortKey, sortDir: $sortDir, from: $from, to: $to, _rev: $rev,){
                totalCount,
                sumOfTotals,
                sumOfBalances,
                edges{
              ...
              sales(first: 100000, offset: $offset, limit: $limit, type: $type, status: $status, sortKey: $sortKey, sortDir: $sortDir, from: $from, to: $to, _rev: $rev,){
                totalCount,
                sumOfTotals,
                edges{
                  node {
              ...
              payments: paymentsOfInvoices(first: 100000, offset: $offset, limit: $limit, type: $type, status: $status, sortKey: $sortKey, sortDir: $sortDir, from: $from, to: $to, _rev: $rev,){
                totalCount,
                sumOfTotals,
                sumOfCredits,
                edges{
...
function loadData({ totalCount, offset, limit, sortKey, sortDir, data: { invoices, sales, payments, }, }){
